...
//TODO dynamic binary
template<typename T,typename Comparator = less>
struct Binary {
  T* buffer;
...
template<typename T>
using MaxBinary = Binary<T,greater>;

template<typename T>
using MinBinary = Binary<T,less>;

template<typename T,typename Comparator ,typename V>
T* insert(Binary<T,Comparator>&,V&&) noexcept;

// template<typename T,typename Comparator ,typename K>
// bool remove(Binary<T,Comparator>&,const K&) noexcept;

template<typename T,typename Comparator ,typename K>
bool take_head(Binary<T,Comparator>&,const K&) noexcept;

template<typename T,typename Comparator >
T* peek_head(Binary<T,Comparator>&) noexcept;

template<typename T,typename Comparator,typename K>
T* find(Binary<T,Comparator>&,const K&) noexcept;//TODO

template<typename T,typename Comparator >
void swap(Binary<T,Comparator>&,Binary<T,Comparator>&) noexcept;//TODO

// create a heap out of given array of elements
template<typename T,typename Comparator >
Binary<T,Comparator>heapify(T*,std::size_t)noexcept;//TODO

// TODO define
// // joining two heaps to form a valid new heap containing all the elements shared by both parties
// template<typename T,typename Comparator >
// bool union(Binary<T,Comarator>&dest,const Binary<T,Comparator>&source) noexcept;//TODO
//
// template<typename T,typename Comparator >
// bool union(Binary<T,Comarator>&dest,const Binary<T,Comparator>&&source) noexcept;//TODO
...
template<typename T,typename Comparator >
bool concat(Binary<T,Comparator>&dest,const Binary<T,Comparator>&source) noexcept;//TODO
...
void
dump(Binary<int,Comparator> &,std::size_t idx=0,std::string prefix="",bool isTail=true,const char *ctx = "") noexcept;
...
namespace heap {
  ...
  static std::size_t parent(std::size_t idx) noexcept {
    return (idx-1)/2;
  }

  static std::size_t left_child(std::size_t idx) noexcept {
    return (2*idx)+1;
  }

  static std::size_t right_child(std::size_t idx) noexcept {
    return (2*idx)+2;
  }

  template<typename T,typename Comp>
  static void swap(Binary<T,Comp>&heap,std::size_t idx1,std::size_t idx2){
    std::swap(heap.buffer[idx1], heap.buffer[idx2]);
  }

  template<typename T,typename Comp>
  static std::size_t sift_up(Binary<T,Comp>&heap,std::size_t idx) noexcept {
Lit:
    if(idx == 0){
  ...
  template<typename T,typename Comp>
  static std::size_t max(Binary<T,Comp>&heap,std::size_t first_idx,std::size_t second_idx) noexcept {
    constexpr Comp c;
    /*
  ...
  template<typename T,typename Comp>
    static void shift_down(Binary<T,Comp>&heap,std::size_t idx) noexcept {
Lit:
      ...
      if(left_idx < heap.length){
        ...
        if(right_idx < heap.length){
          max_idx = max(heap,right_idx,left_idx);
        }
      }
...
: buffer{b},capacity{c},length(0) {
}

template<typename T,typename Comparator ,typename V>
T* insert(Binary<T,Comparator>&heap,V&&val) noexcept {
...
template<typename T,typename Comparator ,typename K>
bool take_head(Binary<T,Comparator>&heap,K&swp) noexcept {
  using namespace impl::heap;
  if(heap.length > 0){
    std::size_t idx=0;
...
template<typename T,typename Comparator>
T* peek_head(Binary<T,Comparator>&heap) noexcept {
  if(heap.length > 0){
...
template<typename T,typename Comparator,typename K>
T* find(Binary<T,Comparator>&,const K&) noexcept {
  return nullptr;//TODO
}

template<typename T,typename Comparator >
void swap(Binary<T,Comparator>&first,Binary<T,Comparator>&second) noexcept {
...
template <typename Comparator>
void
dump(Binary<int,Comparator> &heap,std::size_t idx,std::string prefix,bool isTail,const char *ctx) noexcept {
  using namespace impl::heap;
  if (idx<heap.length) {
    char name[256] = {0};
