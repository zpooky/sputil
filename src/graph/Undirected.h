#ifndef SP_UTIL_GRAPH_UNDIRECTED_H
#define SP_UTIL_GRAPH_UNDIRECTED_H
...
struct Undirected {
  ...
  using edge_type = Wrapper<Undirected<T, N>>;
...
template <typename T, std::size_t N>
static bool
change_owner(Wrapper<Undirected<T, N>> &subject,
             const Undirected<T, N> &owner) {
  assertx(subject.ptr);
  ...
  for (std::size_t i = 0; i < length(subject_edges); ++i) {
    Wrapper<Undirected<T, N>> &edge = subject_edges[i];
    assertx(edge.ptr);

    if (!edge.owner) { // TODO <- document why
      if (edge.ptr != &owner) {
        Wrapper<Undirected<T, N>> *const s =
            bin_search(edge.ptr->edges, subject);
...
cleanup(Undirected<T, N> &self) noexcept {
  ...
  sp::UinStaticArray<Wrapper<Undirected<T, N>>, N> drain(std::move(self.edges));
  ...
  remove_if(drain, [&self](Wrapper<Undirected<T, N>> &current) {
    if (current.owner) {
  ...
  while (!is_empty(drain)) {
    Wrapper<Undirected<T, N>> *const remote = last(drain);
    assertx(remote);
    assertx(remote->ptr);
    ...
    {
      Wrapper<Undirected<T, N>> needle(&self);
      Wrapper<Undirected<T, N>> *const currentToMe =
          bin_search(remote_edges, needle);
      assertx(currentToMe);
...
add_edge(Undirected<T, N> &self, Undirected<T, N> *edge, bool owner) noexcept {
  ...
  const auto search_res =
      bin_search(self.edges, Wrapper<Undirected<T, N>>(edge));
  const std::size_t s_edges = length(self.edges);

  // should only fail on duplicate edge
  auto res = bin_insert_unique(self.edges, Wrapper<Undirected<T, N>>(edge));
  if (res) {
    ...
    {
      auto res2 =
          bin_insert_unique(edge->edges, Wrapper<Undirected<T, N>>(&self));
      assertx(res2);
...
remove_edge(Undirected<T, N> &self, Undirected<T, N> *edge) noexcept {
  ...
  auto res = bin_search(self.edges, Wrapper<Undirected<T, N>>(edge));
  if (res) {
    auto &the_edge = *res;
    if (the_edge.owner) {
    ...
    // TODO owner <-> owner work
    {
      ...
      bool rres = bin_remove(edge->edges, Wrapper<Undirected<T, N>>(&self));
      assertx(rres);
    }
