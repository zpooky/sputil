...
TEST(fnvTest, test_fnv64a) {
  ...
  std::uint64_t  hval = 0xcbf29ce484222325ULL;
    hval = fnv_1a::encode(t->buf, t->len, hval);
    // printf("%lu\n",hval);


    ASSERT_EQ((hval&mask) , (fnv1a_64_vector[tstnum-1].fnv1a_64 &mask));
    }
}
TEST(fnvTest, test_fnv64a_other) {
  const char *str = "test";
    auto hval = fnv_1a::encode(str, 5);
    ASSERT_EQ(std::uint64_t(0xf9e6e6ef197c2b25),hval);
}
