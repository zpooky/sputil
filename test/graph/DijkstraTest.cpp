...
TEST(DijkstraTest, test_impl2) {
  ...
  ASSERT_TRUE(graph::add_edge(v1, 10, &v0));
  ASSERT_TRUE(graph::dijkstra::shortest_path2(&v1, &v0));
  ...
  ASSERT_TRUE(graph::dijkstra::shortest_path2(&v0, &v4));
  ASSERT_TRUE(graph::dijkstra::shortest_path2(&v1, &v4));
  ASSERT_TRUE(graph::dijkstra::shortest_path2(&v2, &v4));
  ASSERT_TRUE(graph::dijkstra::shortest_path2(&v3, &v4));
}
...
TEST(DijkstraTest, test_impl2_asd) {
  ...
  ASSERT_TRUE(graph::dijkstra::shortest_path2(&start, &end));
}
